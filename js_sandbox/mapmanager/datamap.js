...
let { title, first, last } = randomUser.results[0].name
let { birthdate, age } = randomUser.results[0].dob
let fullName = title + ' ' + first + ' ' + last
let { large, medium } = randomUser.results[0].picture
console.log(randomUser.results[0].gender)

document.getElementById('profileDisplay').innerHTML = `
  <ul class='list-group'>
  <li class='list-group-item'>Name : ${fullName}</li>
  <li class='list-group-item'>EMail Address : ${randomUser.results[0].email}</li>
  <li class='list-group-item'>age : ${age}
  </ul>
`
